        ...
        for lineno, line in enumerate(self.content):
            ...
            if isinstance(ST[-1], Equation):
                ...
                if sline.startswith('# '):
                    ...
                    if sline.startswith('$$'):
                    ...
                    elif sline.startswith('!!! definition'):
                        clean()
                        sline = re.sub(r'<a id=".*?"></a>', '', sline).strip()
                        parts = sline.split('"')[1].split(':')
                        chp_num = parts[0].strip().split(' ')[1].split('.')
                        chp = int(chp_num[0])
                        num = int(chp_num[1])
                        title = parts[1].strip()
                        definition = Definition(chp=chp, num=num, title=title, lvl=indent_level)
                        ST[-1].add_content(definition)
                        ST.append(definition)
                        ST.append(definition.contentlist)
                    elif sline.startswith('!!! theorem'):
                        clean()
                        sline = re.sub(r'<a id=".*?"></a>', '', sline).strip()
                        parts = sline.split('"')[1].split(':')
                        chp_num = parts[0].strip().split(' ')[1].split('.')
                        ...
                        num = int(chp_num[1])
                        title = parts[1].strip()
                        theorem = Theorem(chp=chp, num=num, title=title, lvl=indent_level)
                        ST[-1].add_content(theorem)
                        ST.append(theorem)
                        ST.append(theorem.contentlist)
                    elif sline.startswith('!!! concept'):
                        clean()
                        sline = re.sub(r'<a id=".*?"></a>', '', sline).strip()
                        parts = sline.split('"')[1].split(':')
                        chp_num = parts[0].strip().split(' ')[1].split('.')
                        ...
                        num = int(chp_num[1])
                        title = parts[1].strip()
                        concept = Concept(chp=chp, num=num, title=title, lvl=indent_level)
                        ST[-1].add_content(concept)
                        ST.append(concept)
                        ST.append(concept.contentlist)
                    elif sline.startswith('!!! example'):
                        clean()
                        sline = re.sub(r'<a id=".*?"></a>', '', sline).strip()
                        parts = sline.split('"')[1].split(':')
                        chp_num = parts[0].strip().split(' ')[1].split('.')
                        ...
                        num = int(chp_num[1])
                        title = parts[1].strip()
                        example = Example(chp=chp, num=num, title=title, lvl=indent_level)
                        ST[-1].add_content(example)
                        ST.append(example)
                        ST.append(example.contentlist)
