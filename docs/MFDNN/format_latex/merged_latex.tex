...
\begin{definition}[6.2][Data Augmentation (DA)]
    ...
    \begin{itemize}
        ...
        In PyTorch, we use \texttt{Torchvision.transforms}.
    \end{itemize}
...
\begin{concept}[6.10][Weight Decay $\cong \ell^{2}$ - Regularization]
    ...
    or by using \texttt{weight_decay} in the optimizer
...
\begin{concept}[6.13][Dropout in Pytorch]
    ...
    PyTorch provides an implementation of dropout through \texttt{torch.nn.Dropout}.
\end{concept}
...
\begin{concept}[6.26][Using Different Optimizers in Pytorch]
    In PyTorch, the \texttt{torch.optim} module implements the commonly used optimizers.
...
\begin{concept}[6.27][Learning Rate Scheduling]
    ...
    One common choice is to specify a diminishing learning rate via a function (a lambda expression). Choices like \texttt{C/epoch} or \texttt{C / sqrt(iteration)}, where \texttt{C} is an appropriately chosen constant, are common.
...
\begin{concept}[6.44][Backprop in Pytorch]
    ...
    In PyTorch, you (1) clear the existing gradient with \texttt{.zero_grad()}
    (2) forward-evaluate the loss function by providing the input and label and
    (3) perform backprop with \texttt{.backward()}.
    ...
    The \texttt{torch.no_grad()} context manager allows intermediate node values to discarded or not be stored.
...
\begin{definition}[6.46][BN for Linear Layers]
    ...
    where operations are elementwise. BN normalizes each output neuron. The mean and variance are explicitly controlled through learned parameters $\beta$ and $\gamma$. In Pytorch, \texttt{nn.BatchNorm1d}.
\end{definition}
...
\begin{definition}[6.47][BN for Convolutional Layers]
    ...
    BN normalizes over each convolutional filter. The mean and variance are explicitly controlled through learned parameters $\beta$ and $\gamma$. In Pytorch, \texttt{nn.BatchNorm2d}.
\end{definition}
...
\begin{definition}[6.48][BN during Testing]
    ...
    In PyTorch, use \texttt{model.train()} and \texttt{model.eval()} to switch BN behavior between training and testing.
\end{definition}
...
\begin{definition}[8.16][2D Transpose Convolution Layer (Formal Definition)]
    ...
    \begin{itemize}
        ...
        \item Bias $b \in \mathbb{R}^{C_{\text {out }}}$ (If \texttt{bias=False}, then $b=0$.) 
    \end{itemize}
...
\begin{concept}[12.6][Minimax Optimization in Pytorch]
    ...
    The \texttt{OPTIMIZER} can be anything like \texttt{SGD} or \texttt{Adam}.
