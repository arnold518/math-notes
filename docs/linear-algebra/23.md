...
!!! exercise "Exercise 23.15"
    - (a) Let $T$ be a linear operator on a vector space $V$, and let $x$ be an eigenvector of $T$ corresponding to the eigenvalue $\lambda$.
...
!!! exercise "Exercise 23.16"
    ...
    - (b) How would you define the trace of a linear operator on a finite-dimensional vector space?
