...
!!! concept "Concept 32.1 : Projections and direct-sum decomposition."
    Recall from the exercises of Section 2.1 that if $V=W_{1} \oplus W_{2}$, then a linear operator $T$ on $V$ is the projection on $W_{1}$ along $W_{2}$ if, whenever $x=x_{1}+x_{2}$ with $x_{1} \in W_{1}$ and $x_{2} \in W_{2}$, we have $T(x)=x_{1}$.
    By **Exercise 26 of Section 2.1**, we have
    ...
    In fact, it can be shown (see **Exercise 17 of Section 2.3**) that $T$ is a projection if and only if $T=T^{2}$.
...
!!! theorem "Theorem 32.3 : Uniqueness of orthogonal projection on a subspace."
    Now assume that $W$ is a finite-dimensional subspace of an inner product space $V$.
    In the notation of **Theorem 28.17**, we can define a function $T: V \rightarrow V$ by $T(y)=u$.
    It is easy to show that $T$ is an orthogonal projection on $W$.
    We can say even more: there exists exactly one orthogonal projection on $W$.
    For if $T$ and $U$ are orthogonal projections on $W$, then $R(T)=W=R(U)$.
    Hence $N(T)=R(T)^{\perp}=R(U)^{\perp}=N(U)$, and since every projection is uniquely determined by its range and null space, we have $T=U$.
...
!!! example "Example 32.4 : Geometric difference between projection and orthogonal projection."
    ...
    ![](./assets/32.1.jpg){: .center style="width:100%;"}
    ///caption
    Figure 32.1.
    ///
...
!!! example "Example 32.5 : Fourier approximation via orthogonal projection."
    As an application to Fourier analysis, recall the inner product space $H$ and the orthonormal set $S$ in Example 9 of Section 6.1.
...
!!! theorem "Theorem 32.6 : Algebraic characterization of orthogonal projections."
    ...
    !!! proof
        ...
        Then $T$ is a projection by **Exercise 17 of Section 2.3**, and hence we must show that $R(T)=N(T)^{\perp}$ and $R(T)^{\perp}=N(T)$.
...
!!! theorem "Theorem 32.8 : Spectral theorem."
...
!!! concept "Concept 32.9 : Block-diagonal matrix form and polynomial calculus."
    With the preceding notation, let $\beta$ be the union of orthonormal bases of the $W_{i}$'s and let $m_{i}=\operatorname{dim}\left(W_{i}\right)$.
...
!!! corollary "Corollary 32.11 : Normality and polynomial representation of adjoint."
    ...
    !!! proof
        ...
        Using the Lagrange interpolation formula (see page 52), we may choose a polynomial $g$ such that $g\left(\lambda_{i}\right)=\overline{\lambda}_{i}$ for $1 \leq i \leq k$.
        Then
