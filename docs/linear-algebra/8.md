...
!!! example "Example 8.3 : Rotation by $\theta$ is linear."
    ...
    Let $\alpha$ be the angle that $\left(a_{1}, a_{2}\right)$ makes with the positive $x$-axis, and let $r=\sqrt{a_{1}^{2}+a_{2}^{2}}$.
...
!!! theorem "Theorem 8.14 : One-to-one if and only if null space is trivial."
    Let $V$ and $W$ be vector spaces, and let $T: V \rightarrow W$ be linear.
...
!!! theorem "Theorem 8.15 : One-to-one, onto, and full rank in equal finite dimensions."
    ...
    !!! proof
        ...
        Now, with the use of **Theorem 8.14**, we have that $T$ is one-to-one if and only if $N(T)=\{0\}$, if and only if $\operatorname{nullity}(T)=0$, if and only if $\operatorname{rank}(T)=\operatorname{dim}(V)$, if and only if $\operatorname{rank}(T)=\operatorname{dim}(W)$, and if and only if $\operatorname{dim}(R(T))=\operatorname{dim}(W)$.
